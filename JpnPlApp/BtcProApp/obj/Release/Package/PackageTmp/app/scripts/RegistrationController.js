...
module.controller('Register', function ($scope, $http, $location, $mdDialog) {
    ...
    $scope.checkEmailId = function () {
        ...
        if ($scope.registerModel.EmailId != "") {
            //$http.get("IsEmailPresent?EmailId=" + $scope.registerModel.EmailId).then(function (data) {
                ...
                $http.post('/api/Registers', $scope.registerModel).then(function (data) {
                    ...
                    $http.post('/Home/SendMyRegistrationEmail?Username=' + $scope.registerModel.UserName).then(function (response) {
                        if (response.data.status == 'Success') {
                            $scope.showEmail = false;
                            $http.post("/Home/LedgerPostingMember?Username=" + $scope.registerModel.UserName + "&WalletType=5&Amount=" + $scope.payamount).then(function (response) {
                                if (response.data.Success) {
                                    $http.post("AutoPurchase?username=" + $scope.registerModel.UserName + "&UplineId=" + $scope.registerModel.ReferrerId + "&packageId=1&investmentAmt=" + $scope.payamount+"&mode=Form").then(function (data) {
                                        debugger;
    ...
    $scope.checkLoginUsername = function () {
        ...
        if ($scope.loginModel.UserName != "") {
            $http.get("/Account/IsUserNameExist1?UserName=" + $scope.loginModel.UserName).then(function (data) {
    ...
    $scope.showConfirm = function (ev) {
        ...
        var confirm = $mdDialog.confirm()
              ...
              .textContent('Check your registered email id if you proceed.')
              .ariaLabel('Proceed')
              .targetEvent(ev)
              .ok('Proceed to reset!')
